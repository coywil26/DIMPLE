    ...
    def __init__(self, gene, start=[], end=[]):
        ...
        if self.stop_codon:
            self.aminoacids = ['Cys', 'Asp', 'Ser', 'Gln', 'Met', 'Asn', 'Pro', 'Lys', 'Thr', 'Phe', 'Ala', 'Gly',
                               'Ile', 'Leu', 'His', 'Arg', 'Trp', 'Val', 'Glu', 'Tyr', 'STOP']
        else:
            self.aminoacids = ['Cys', 'Asp', 'Ser', 'Gln', 'Met', 'Asn', 'Pro', 'Lys', 'Thr', 'Phe', 'Ala', 'Gly',
                               'Ile', 'Leu', 'His', 'Arg', 'Trp', 'Val', 'Glu', 'Tyr']
...
def find_geneprimer(genefrag, start, end):
    ...
    count = 0
    while tm2 < DIMPLE.gene_primerTm[0] or tm2 > DIMPLE.gene_primerTm[1] or tm4 < DIMPLE.gene_primerTm[0] or tm4 > DIMPLE.gene_primerTm[1]:
    ...
    for ii, gene in enumerate(OLS):
        ...
        while idx < len(gene.breaklist):
            ...
            if gene.unique_Frag[idx]:  # only for unique sequences
                ...
                gene.barPrimer.append(SeqRecord(primerF, id=gene.geneid + "_oligoP_DMS-" + str(idx + 1) + "_F",
        ...
        SeqIO.write(gene.genePrimer, os.path.join(folder.replace('\\', ''),
                                                  gene.geneid + "_DMS_Gene_Primers.fasta"), "fasta")
